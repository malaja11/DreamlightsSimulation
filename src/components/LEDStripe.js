...
export default function LEDStripe({count, radius, x, y}) {
    ...
    return [...Array(count)].map((e, i) => {
        const componentLED = <div className='led' key={i} id={"led" + i} style={{ left: currentLED.x, top: currentLED.y, width: currentLED.r * 2, height: currentLED.r * 2 }} onMouseDown={(e) => onMouseDown(e)} ></div>;
    ...
    function moveLED(div, x, y) {
        div.style.left = x + "px";
        div.style.top = y + "px";
        const id = parseInt(div.id.substring(3));

        if (id !== 0) {
            moveLEDNextTo(document.getElementById("led" + (id - 1)), x, y, -1);
        }

        const next = document.getElementById("led" + (id + 1));
        if (next) {
    ...
    function moveLEDNextTo(div, x, y, diff) {
        ...
        const nextId = "led" + (parseInt(div.id.substring(3)) + diff);
